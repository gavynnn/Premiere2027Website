...
| Question / output limit | 400 characters / 400 ordinary output tokens; 750 for sponsorship/detail (hard cap 800) |
| Retrieval / context limit | 3 chunks per search, 1 tool call, last 5 exchanges (10 prior messages); one PDF input on visual questions |
| Reply length | Normally 1–3 short sentences, up to 75 words; sponsorship/detail up to 8 short sentences or bullets and 180 words |
...
`server/reply-language.mjs` resolves clear EN/ID intent before prompting; the website language is only a fallback. Ordinary replies are brief, friendly chat, with useful next-step buttons. Sponsorship and requested detail use spaced headings and bullets, with a server-side formatting fallback. A request for a short answer overrides the detailed mode. Footer contact links also open WhatsApp; no phone numbers were changed.

`server/reply-links.mjs` removes raw citation glyphs, Markdown link wrappers, generated URLs and internal file IDs. The model selects action intents, not destinations. The UI constructs actual Register/Closing Night/WhatsApp anchors and published proposal attachments, with download and website-view alternatives. Missing e-invites never get an invented file link. The popup is 492px wide (20% wider), with an 828px preferred height constrained to the viewport and desktop navigation; the launcher is roughly one-third larger. Chat rows use the same Astra icon as the launcher and a default user avatar.

### Visual document knowledge

For questions about images, artist logos, charts or placement diagrams, `server/document-context.mjs` selects one current sponsor PDF as a Responses `input_file`. [OpenAI PDF inputs include both text and page images](https://developers.openai.com/api/docs/guides/file-inputs). This costs more input tokens than text search, so ordinary turns do not resend the PDF. No extra upload/index is required for the already-uploaded file IDs.

The same module contains reviewed notes from physical PDF pages 3, 11, 14–16 and 20–24, including artist logos and the historical audience chart. Notes are enabled only while a reviewed PDF hash matches the active manifest. Re-review these notes when replacing documents and run the normal document sync. Past guest stars and target guests are not confirmed 2027 bookings; approximate historical audiences are not competitor totals, unique visitor sums or guaranteed future reach. The proposal's approximately 200 schools is attributed, not claimed as verified 2027 registration data. Tier-dependent benefits and live package availability must stay distinct.
...
The newer 12-call experience evaluation is `node tools/check-chat-experience.mjs --paid` (requires explicit spend approval). It includes two PDF-vision calls, writes a report, and refuses to repeat an existing run silently. See `tests/reports/chat-experience.md` for the mixed-revision results and known limitations. `tools/check-chat-ui.mjs` tests the UI with mocked answers only, including downloads, safe links, avatars, mobile sizes and persistent navigation. It requires Playwright and installed Chrome; set `PLAYWRIGHT_PACKAGE` to a bundled package path if needed. Neither this work nor running tests deploys the site.
